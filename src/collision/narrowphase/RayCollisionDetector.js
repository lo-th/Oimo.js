 ...
 * @author xprogram
 */
OIMO.RayCollisionDetector = function(){
...
OIMO.RayCollisionDetector.prototype.detectCollision = function(shape1, shape2, manifold){
    var pos1 = shape1.position;
    var pos2 = shape2.position;
    var vec3_1 = new OIMO.Vector3(pos1.x, pos1.y, pos1.z);
    var vec3_2 = new OIMO.Vector3(pos2.x, pos2.y, pos2.z);
    var intersect;
    ...
    for(var i = 0; i < shape2.faces.length; i++){
        ...
        if(intersect !== null)
            manifold.addPoint(new OIMO.Vec3(intersect.x, intersect.y, intersect.z));
    }
};
...
function triangleIntersect(origin, direction, face, backfaceCulling){
	...
	return new OIMO.Vector3().copy(direction).multiplyScalar(QdN / DdN).add(origin);
}
